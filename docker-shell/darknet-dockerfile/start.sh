...
if [ -f "assets/train_log/convert_data.log" ];then
        echo "已经转换成功，不需要重复转换" > "assets/train_log/reain_docker.log" 2>&1
else
...
./darknet detector train assets/voc.data assets/yolov3-voc.cfg assets/backup/yolov3-voc_last.weights -dont_show -gpus $use_gpus -map >> "assets/train_log/reain_docker.log" 2>&1 
else
echo "yolov3-voc_last.weights not exist, use darknet53.conv.74" >> "assets/train_log/reain_docker.log" 2>&1
./darknet detector train assets/voc.data assets/yolov3-voc.cfg darknet53.conv.74 -dont_show -gpus $use_gpus -map >> "assets/train_log/reain_docker.log" 2>&1
fi
ok=$?
if [ $ok == 0 ];then

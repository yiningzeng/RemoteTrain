...
class powerAi2coco(object):
    ...
    def firstAddCategories(self):
        firstJson = json.load(open("/detectron2/datasets/coco_categories.cjson", encoding='utf-8'))
        for item in firstJson["categories"]:
            categorie = {}
            categorie['supercategory'] = 'component'
            categorie['id'] = item['id']
            categorie['name'] = item['name']
            self.categories.append(categorie)
            self.label.append(item['name'])

    def save_json(self):
        #1.最先需要加载cjson里的标签数据，否则标签的id可能会乱
        self.firstAddCategories()
        self.data_transfer()
